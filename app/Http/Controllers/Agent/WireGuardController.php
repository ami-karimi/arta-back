...
class WireGuardController extends Controller
{
    public function index(){
        ...
        return response()->json([
            'status' => true,
            'ras' => Ras::select(['name','id','unlimited'])->where('unlimited',1)->where('is_enabled',1)->get(),
            'groups' => Groups::select(['group_type','name','multi_login','id'])->where('name','like','%وایرگارد%')->get(),
            'admins' => [],
        ]);
    ...
    public function update($id,Request $request){
        ...
        if($find->user->creator !== auth()->user()->id){
            return response()->json(['status' => false,'message' => 'کاربر یافت نشد!'],403);
        }
